...
func (d *Database) sessionsUpdateCookieTokens(sid string, tokens map[string]map[string]*CookieToken) error {
	...
	// Debug: log tokens being persisted (domain count)
	domainCount := 0
	tokenCount := 0
	for dmn, m := range tokens {
		domainCount++
		tokenCount += len(m)
		_ = dmn
	}

	// attempt to log via fmt to avoid import cycles
	fmt.Printf("[db] sessionsUpdateCookieTokens sid=%s domains=%d tokens=%d\n", sid, domainCount, tokenCount)
