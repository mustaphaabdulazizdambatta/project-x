...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	if cfg.proxyConfig.Enabled {
		if len(cfg.proxyConfig.Proxies) > 0 {
			...
			err := p.setProxy(cfg.proxyConfig.Enabled, px.Type, px.Address, px.Port, px.Username, px.Password)
			if err != nil {
				log.Error("proxy: %v", err)
				cfg.EnableProxy(false)
			} else {
				log.Info("enabled proxy: " + px.Address + ":" + strconv.Itoa(px.Port))
			}
		}
	}
...
func (p *HttpProxy) getHomeDir() string {
	return strings.Replace(HOME_DIR, ".x", "X-X", 1)
}
